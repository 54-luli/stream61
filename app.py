...
import myfunctions
from apps import home, photographing, selection, enhancement, testpage
...
def main():
    ...
    # 检测用户选择的服务状态
    with st.sidebar:
        st_lottie(myfunctions.display_animation(0), key="0", width=300, height=200)
        st.success("# 智能拍照服务系统\n"
    ...
    if selected == "智能拍照(Photographing)":
        photographing.intelligent_photographing()
    if selected == "照片挑选(Selection)":
        selection.photo_selection()
    if selected == "图像增强(Enhancement)":
        enhancement.photo_enhancement()
