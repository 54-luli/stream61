...
def background():
    photo_path = os.path.join(myfunctions.PWD, 'test/images/mountain.jpg')
    # 加载背景图
...
def web_home():
    ...
    with p11:
        st.map(map_data)
    with p12:
        st.image(myfunctions.show_image("test/images/pi.png"))
        st.image(myfunctions.show_image("test/images/mypi.jpg"), width=450)
